...
const Stats = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row px-2  space-y-10 md:space-y-0 justify-around items-center py-10">
      <div className="text-center" ref={ref20}>
        <div className="  text-6xl md:text-7xl ld:text-8xl font-bold text-gray-800">
          {inView20 && <CountUp end={20} duration={2} suffix="%" />}
        </div>
        <div className="text-gray-600 text-sm">Increase in conversions over the first 6 months</div>
      </div>
      <div className="text-center" ref={ref30}>
        <div className="text-6xl md:text-7xl ld:text-8xl font-bold text-gray-800">
          {inView30 && <CountUp end={30} duration={2} suffix="%" />}
        </div>
        <div className="text-gray-600 text-sm ">Increase in Search Impression Share on Average</div>
      </div>
      <div className="text-center" ref={ref200M}>
        <div className="text-6xl md:text-7xl ld:text-8xl font-bold text-gray-800">
          {inView200M && <CountUp end={200} duration={2} prefix="$" suffix="M+" />}
        </div>
        <div className="text-gray-600 text-sm ">In online sales and MQLs generated</div>
      </div>
    </div>
  );
};
